...
class ModelShippingLitemfDelivery extends Model {
	...
	public function addOrderLitemf($data) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "litemf_address` SET user_id = '" . (int)$data['user_id'] . "', first_name = '" . $data['passport']['first_name'] . "', last_name = '" . $data['passport']['last_name'] . "', middle_name = '" . $data['passport']['middle_name'] . "', street = '" . $data['passport']['street'] . "', house = '" . $data['passport']['house'] . "', city = '" . $data['passport']['city'] . "', region = '" . $data['passport']['region'] . "', zip_code = '" . $data['passport']['zip_code'] . "', phone = '" . $data['passport']['phone'] . "', series = '" . $data['passport']['series'] . "', number = '" . $data['passport']['number'] . "', issue_date = '" . $date . "', issued_by = '" . $data['passport']['issued_by'] . "'");
		$litemf_address_id = $this->db->getLastId();
		$this->db->query("INSERT INTO `" . DB_PREFIX . "litemf_orders` SET litemf_address_id = '" . (int)$litemf_address_id . "' status = 'unsend', user_id = '" . (int)$data['user_id'] . "', order_id = '" . $this->db->escape($data['order_id']) . "', delivery_method_id = '" . $data['passport']['delivery_method_id'] . "', delivery_point_id = '" . $poinId . "'");
